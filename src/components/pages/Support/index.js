...
const SupportPage = ({ classes }) => {
  ...
  const handleSubmit = (e) => {
    ...
    db.collection('reports').doc().set({email, content})
  ...
  return (
    <div className={classes.page}>
      ...
      <form className={classes.form} onSubmit={handleSubmit}>
        ...
        <textarea type="text" placeholder="Need Help?" rows="5" value={content} onChange={e => setContent(e.target.value)} />
        <button type="submit">Submit</button>
      </form>
      {status.length > 0 && (
       <h2 className={status.includes('Error:') ? classes.error : classes.success}>{status}</h2>
      )}
    </div>
  );
};
